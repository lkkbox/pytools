...
from ..readtools import readtools as rt
from ..readtools.era5_sfc import readClim as era5_sfc_clim
...
def readTotal(modelName, varName, minMaxs, members=[0]):
    ...
    fileName = '/nwpr/gfs/com120/9_data/models/processed/{modelName}/' \
             + '{year:02d}/{month:02d}/{day:02d}z00/E{member:03d}/global_1p0_{varName}.nc'
    ...
    for iInit in range(numInits):
        ...
        for iMember, member in enumerate(members):
            fp.flushPrint(f' ({iInit+1}/{numInits}) ({iMember+1}/{numMembers})')
            f = fileName.format(modelName=modelName, year=year, month=month, day=day, varName=varName, member=member)
            ...
            var[iMember, iInit, 0:numLeadSlice, :] = varSlice
    fp.flushPrint('')
    var = np.nanmean(var, axis=0) # ensemble average
...
def readModelClim(modelName, varName, minMaxs, climYears=[2011, 2020], climType='5dma'):
    ...
    for iMonth in range(numMonths):
        ...
        # when no data is retreived between minmaxs
        if varSlice is None:
            print(f'[readModelClim] warning: No data is read for {minMaxs = }')
            continue

        numInitSlice = varSlice.shape[0]
        numLeadSlice = varSlice.shape[1]
        if numLeadSlice < numLeads:
            print(f'[readModelClim] warning: numLead is only {numLeadSlice} for {month=} (inquired lead={numLeads})')
...
def readModelAnomaly(modelName, varName, minMaxs, climData, climYears=[2011, 2020], climType='5dma', members=[0]):
    ...
    if climData == 'model':
        ...
        if varName in ['u10', 'v10', 't2m', 'pw', 'mslp']:
            varClim, dimClim = era5_sfc_clim(varName, minMaxX, minMaxY, minMaxT, climYears, climType)
        elif varName in ['u', 'v', 'z', 'vp', 'sf']:
    ...
    if varClim is None:
        print('ERROR: failed to read model climate data')
        return None, None

    varTotal, dimTotal = readTotal(modelName, varName, minMaxs, members=members)
    if varTotal is None:
    ...
    if climData == 'obs': # then interpolation is needed
        ...
        varAnomaly = subtractObsClim(varTotal, dimTotal, varClim, dimClim[0])
    elif climData == 'model':
        varAnomaly = subtractModelClim(varTotal, dimTotal[0], varClim, dimClim[0])
    return varAnomaly, dimTotal


def subtractModelClim(varTotal, timeTotal, varClim, timeClim):
    ...
    return varTotal - varClim[iInitList, :]


def subtractObsClim(varTotal, dimTotal, varClim, timeClim):
    inits = dimTotal[0]
    leads = dimTotal[1]
    varAnomaly = varTotal
    ...
    for iInit, init in enumerate(inits):
        for iLead, lead in enumerate(leads):
            valid = float(init + lead)
            iValid = np.floor(valid - tt.ymd2float( tt.year(valid), 1, 1))
            iClim = timeClim.index(iValid)
