    ...
    def getOneModel(modelName):
        ...
        def mergeFilesToNC(srcFiles):
            ...
            if modelSetting['levels'][varName] is None:
                ...
                numEntries = len(modelSetting['levels'][varName])
                doWNP = False
            ...
            for iFile, srcFile in enumerate(srcFiles):
                ...
                if modelSetting['dataStructure'] == 'mergedGrib2':
                    ...
                    # checking the number of entries retreived by wgrib2 matching the number of levels
                    ...
                    # evaluating the number of entires retrieved
                    if result is None:  # no output by wgrib2
                        numRetrievedEntries = 0
                    else:
                        lines = result.split('\n')
                        entries = [e for e in lines if e != ''] # somehow an empty string can be returned from wgrib2..

                        # remove the wgrib2 line number
                        # and check if there are duplicated entries
                        # (because prec in TGFS 0-6h are duplicated, and 
                        #  messed up the counting...)
                        contents = [e.split(':')[1:] for e in entries]
                        iUniqueEntry = [i for i, c, in enumerate(contents) if c[i] not in c[i+1:]]
                        entries = [entries[i] for i in iUniqueEntry]
                        numRetrievedEntries = len(entries)
                        

                    # quit looping if the entry number is wrong
                    if numRetrievedEntries == 0:
                        logging.error('\nFAIL: received 0 entries:\n'
                                        + f'command = {cmd_getEntries}\n'
                                        + 'entries = None')
                        break
                    elif numRetrievedEntries != numEntries:
                        logging.error(f'\nFAIL: received {numRetrievedEntries} entries'
                                        + f'( expecting {numEntries})\n'
                                        + f'command = {cmd_getEntries}\n'
                                        + f'entries=\n    {'\n    '.join(entries)}')
                        break
