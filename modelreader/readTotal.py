...
'''
from ._shared import ModelFile, checkNDim
...
def getMaxNumLeads(
    ...
    checkType(members, list, 'memebers')
    ...
    for member in members:
        for initTime in initTimes:
            mf = ModelFile(modelName, dataType, varName, initTime, member, warning, rootDir)
            time = mf._readDimValue(lead_name, [-np.inf, np.inf])
            if time is None:
                continue
            if len(time) > maxNumLeads:
                maxNumLeads= len(time)
...
def readTotal(
    ...
    #
    # ---- file settings and checking
    #
    fp.flush('validating input arguments and files..')
    modelFiles = [
        [
            ModelFile(
                modelName, dataType, varName, initTime, member, warning, rootDir
            )
            for initTime in initTimes
        ] 
        for member in members
    ...
    for iInitTime in range(numInitTimes):
        for iMember in range(numMembers):
            fp.flush(
                f'reading {varName} total {iMember}/{numMembers}, {iInitTime}/{numInitTimes}')
